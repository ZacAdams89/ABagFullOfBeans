...
- (void)viewDidLoad
{
    ...
    [cancelButton setTitleColor:C_BLACK forState:UIControlStateNormal];
    [resetButton setTitleColor:C_BLACK forState:UIControlStateNormal];
...
//        if ([self pinLenght] == _inputPin.length && [self checkPin:_inputPin]) {
        if ([self pinLenght] == _inputPin.length) {
            ...
            [self checkPin:_inputPin withResultBlock:^(BOOL valid){
...
- (void)checkPin:(NSString *)pinString withResultBlock:(void (^)(BOOL))resultBlock {
    if(self.delegate && [self.delegate respondsToSelector:@selector(checkPin:withResultBlock:)]) {
        [self.delegate checkPin:pinString withResultBlock:resultBlock];
    }
}
...
//    [SupaTOOLSAppDelegate.window.rootViewController addChildViewController:self];
//    [SupaTOOLSAppDelegate.window.rootViewController.view addSubview:self.view];
}

- (void)popIn:(BOOL)popIn{
    for(UIView *circleView in pins.subviews){
        [self popView:circleView popIn:popIn];
    }
    
    for(UIView *view in _circleViewList){
        [self popView:view popIn:popIn];
    }
    
    [self popView:resetButton popIn:popIn];
    [self popView:cancelButton popIn:popIn];
    [self popView:pinLabel popIn:popIn];
}

- (void)popView:(UIView *)view popIn:(BOOL)popIn{
...
//
//    
//    CGFloat speed = (CGFloat)(((rand() % 10)+5)/10.0f);
//    
//    [UIView animateWithDuration:speed delay:0.0f usingSpringWithDamping:0.7f initialSpringVelocity:0.7f options:UIViewAnimationOptionCurveEaseInOut animations:^{
//        
//        if(popIn){
//            view.scale = 1.0f;
//        }
...
//    for(UIView *circleView in pins.subviews){
//        
//        if([circleView isKindOfClass:[PPCircleButton class]]){
////         CGFloat speed = (CGFloat)(((rand() % 10)+5)/10.0f);
//             circleView.scale = 1;
//               CGFloat speed = (CGFloat)(((rand() % 10)+5)/10.0f);
//            
////             [UIView animateWithDuration:1 delay:speed*100 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
////                 
////                 circleView.scale = 0;
////                 
////             } completion:^(BOOL finished) {
...
////             }];
//            
//            [UIView animateWithDuration:speed delay:0 usingSpringWithDamping:0.7f initialSpringVelocity:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
//                circleView.scale = 0;
//            } completion:^(BOOL finished) {
//                
//            }];
